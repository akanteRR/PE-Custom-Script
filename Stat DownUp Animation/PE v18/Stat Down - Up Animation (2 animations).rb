...
PluginManager.register({
  ...
  :version => "1.0",
  :credits => ["KleinStudio for the original","bo4p5687 for update"]
})
#-------------------------------------------------------------------------------
if defined?(KLEIN_BFUNCS_WORKING) && defined?(klein_bitmap_version) && klein_bitmap_version>=1
  class PokeBattle_Scene
    alias klein_oldcommonanim pbCommonAnimation
    def pbCommonAnimation(animName,user=nil,target=nil)
      if user != nil && (animName=="StatDown" || animName=="StatUp")
        stat = BitmapCache.load_bitmap("Graphics/Pictures/Stats Animation/" + animName) if stat.nil?
        opacity = 255
        (127/4).times do
      ...
      klein_oldcommonanim(animName,user,target)
    end
  end
end
...
#-------------------------------------------------------------------------------
  ...
  def animationItem(item,pkmn)
    ...
    @pkmn = pkmn
    # Position
    middlex = Settings::SCREEN_WIDTH/2
    middley = Settings::SCREEN_HEIGHT/2
    # Background
    create_sprite("bg","Background Item",@viewport)
    # Set pokemon
		filename = GameData::Species.front_sprite_filename( (@pkmn.species), (@pkmn.form rescue 0), (@pkmn.gender), (@pkmn.shiny?), (@pkmn.shadowPokemon?) )
    ...
    @sprites["item"].bitmap = Bitmap.new(filename)
  ...
  def animationTMHM(item,pkmn,name,oldmove,movename,four,&block)
    ...
    @sprites["overlay"].bitmap.fill_rect(0,0,Settings::SCREEN_WIDTH,Settings::SCREEN_HEIGHT,Color.new(213,255,254))
